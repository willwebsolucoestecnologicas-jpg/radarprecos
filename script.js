// script.js - v8.5 (Versão Expandida: Correção de Cliques + Carrinho Oculto no Chat)
...
// --- NAVEGAÇÃO (AQUI ESTÁ A LÓGICA DE ESCONDER O CARRINHO) ---
...
function renderizarCarrinho() {
    ...
    carrinho.forEach(item => {
        ...
        div.innerHTML = `
            ...
            <button onclick="removerDoCarrinho('${item.id}')" class="text-red-400 p-2">
...
// --- UTILITÁRIO: NOTIFICAÇÃO (CORRIGIDO PROBLEMA DE CLIQUE) ---
function mostrarNotificacao(msg, tipo = 'sucesso') {
    ...
    // Define a cor e ATIVA os cliques (pointer-events-auto)
    ...
    // Esconde depois de 3 segundos e DESATIVA cliques (pointer-events-none)
    setTimeout(() => {
...
async function iniciarCamera(modo) {
    ...
    if (modo === 'pesquisar') {
        trocarAba('registrar'); 
        document.querySelector('#scanner-section h2').textContent = "Escanear";
...
async function carregarCatalogo() {
    ...
    try { 
        ...
        lista.innerHTML = `<div class="text-center py-10 opacity-50"><p>Erro conexão.</p></div>`; 
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Adicionando ouvintes de eventos para botões críticos
