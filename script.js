// script.js - v8.7 (Câmera Modal Global + Correções Finais)
...
let modoScanAtual = 'registrar'; // 'registrar' ou 'pesquisar'
...
async function trocarAba(aba) {
    ...
    // Fecha câmera se estiver aberta ao trocar de aba
...
function limparCarrinho() {
    if(confirm("Limpar lista?")) { carrinho = []; salvarCarrinho(); renderizarCarrinho(); toggleCarrinho(); }
}
...
async function onScanSuccess(decodedText) {
    fecharCamera(); // Fecha o modal imediatamente
    
    if (modoScanAtual === 'pesquisar') {
        // Se estava na busca, preenche e busca
        trocarAba('consultar');
        document.getElementById('ean-busca').value = decodedText;
        pesquisarPrecos();
    } else {
        // Se estava registrando, vai pro form
        trocarAba('registrar');
...
async function salvarPreco(e) {
    e.preventDefault(); const btn = e.target.querySelector('button[type="submit"]'); const txt = btn.innerHTML; btn.innerHTML = '...'; btn.disabled = true;
...
async function enviarMensagemGemini() {
    const input = document.getElementById('chat-input'); const area = document.getElementById('chat-messages'); const txt = input.value.trim(); if (!txt) return;
    area.innerHTML += `<div class="chat-user text-sm mb-2">${txt}</div>`; input.value = ''; area.scrollTop = area.scrollHeight;
    const id = 'load-' + Date.now(); area.innerHTML += `<div id="${id}" class="chat-ai text-sm mb-2 opacity-50"><i class="fas fa-circle-notch fa-spin"></i> Digitando...</div>`; area.scrollTop = area.scrollHeight;
    try { const res = await fetch(`${APPS_SCRIPT_URL}?acao=chatGemini&pergunta=${encodeURIComponent(txt)}`, { redirect: 'follow' }); const data = await res.json(); document.getElementById(id).remove(); if (data.resposta) { const r = data.resposta.replace(/\n/g, '<br>').replace(/\*\*(.*?)\*\*/g, '<b>$1</b>'); area.innerHTML += `<div class="chat-ai text-sm mb-2">${r}</div>`; } else { area.innerHTML += `<div class="chat-ai text-sm mb-2 text-yellow-400">Sem resposta.</div>`; } } catch (e) { if(document.getElementById(id)) document.getElementById(id).remove(); area.innerHTML += `<div class="chat-ai text-sm mb-2 text-red-400">Erro de conexão.</div>`; } area.scrollTop = area.scrollHeight;
}
